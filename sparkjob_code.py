...
#delta
spark.sparkContext.addPyFile("s3://sujitha-s3-landingzone-ohio/Files/delta-core_2.12-0.8.0.jar")
...
class data_transformation:
    def __init__(self,spark_config_path,datasetName,dataset_path):
        ...
        #setting the spark configuration from spark config file
        self.setSparkConfig(spark_config_path)
        
        #reading the contents of app config file
        self.jsonData = self.read_config()
        
        #source and destination location raw zone data
    ...
    def read_data_from_rawzone(self, path,format):
        try:
            if format == "parquet":
    ...
    def writing_data_to_stagingzone(self, df,path,format,partition_columns):
        try:
            if format == "parquet":
...
#creating object   
sparkjob=data_transformation(spark_config_path,datasetName,dataset_path)

#Reading data from raw zone
rawzone_df = sparkjob.read_data_from_rawzone(sparkjob.raw_source,sparkjob.raw_source_format)

#casting raw zone data            
casted_df = sparkjob.casting(rawzone_df,sparkjob.transformation_columns)

#masking raw zone data
df = sparkjob.masking(casted_df, sparkjob.masking_columns)
        
#Creating a lookup dataset        
df = sparkjob.lookup_dataset(df,sparkjob.lookup_location,sparkjob.pii_cols,datasetName)
        
#Writing the data to Staging zone after transformation
sparkjob.writing_data_to_stagingzone(df,sparkjob.raw_destination,sparkjob.raw_destination_format,sparkjob.partition_columns)
